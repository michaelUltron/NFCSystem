...
export function OrderPage() {
  ...
  const [initializingGuestMode, setInitializingGuestMode] = useState(true);
  ...
  useEffect(() => {
    const enforceGuestCheckout = async () => {
      try {
        // Force guest mode for all users on the order page
        await supabase.auth.signOut();
      } catch {
        // Ignore sign-out errors here; page should still behave as guest flow
      } finally {
        setInitializingGuestMode(false);
      }
    };

    enforceGuestCheckout();
  }, []);
      ...
      <div className="max-w-7xl mx-auto p-6">
        ...
        <div className="rounded-lg border border-blue-200 bg-blue-50 p-4 text-sm text-blue-900 mb-6 flex items-start gap-3">
          ...
          <div>
            ...
            <p>
              Card ordering is processed as a guest checkout flow. Payment is one-time only,
              and your cards will be prepared after payment confirmation.
            </p>
          </div>
        </div>

        {loadingCatalog || initializingGuestMode ? (
